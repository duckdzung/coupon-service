...
public class RedisDataController {
    ...
    private static final int CHUNK_SIZE = 10 * 1024; // 10KB chunks for memory efficiency
    ...
    public ResponseEntity<ApiResponse> createLargeData(@PathVariable int sizeMB) {
        try {
            ...
            // Calculate chunk info for memory efficiency
            String chunkSuffix = "Data chunk 999999 | "; // Max size estimation
            int chunkSuffixSize = chunkSuffix.getBytes(StandardCharsets.UTF_8).length;
            int fullPatternSize = basePatternSize + chunkSuffixSize;
